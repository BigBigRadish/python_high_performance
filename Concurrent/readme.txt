...
生成器例子gen_demo.py
六.将同步代码重写为异步代码
将阻塞调用与asyncio兼容，对于阻塞代码，处理策略是在一个独立的线程中运行它们。线程是os层级实现的，允许阻塞代码并行执行，python接口Executor，它被设计用与
在独立的线程中运行任务，使用future监视任务进度。初始化线程池，从concurrent.futures导入。任务结束后，执行器不会销毁相应线程，降低创建与销毁的开销。
如threadpool_dmeo.py
七.使用Rx	Py进行响应式编程
响应式编程是一种编程范式，目的是开发并发系统
响应速度快，伸缩性高，富有弹性，消息驱动。
响应式编程将事件视为数据流，详见rx_demo.py
八.在处理速度缓慢且无法预测的资源时，异步编程很有用。
在处理多个彼此关联的资源时，asyncio很有用，避免回调函数，简化了代码逻辑。响应式编程擅长的是处理实时应用程序和用户界面常见的数据流。
